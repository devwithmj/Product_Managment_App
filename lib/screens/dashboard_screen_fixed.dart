...
class _DashboardScreenState extends State<DashboardScreen> {
  ...
  Future<void> _loadDashboardData() async {
    try {
      ...
      setState(() {
        ...
        _updatedTodayProducts = products.where((p) => 
          p.updatedAt.isAfter(todayStart)).length;
  ...
  Future<void> _handleBarcodeScanning() async {
    ...
    if (scannedBarcode != null && scannedBarcode.isNotEmpty) {
      ...
      try {
        final products = await _databaseService.getProducts();
        final foundProduct = products.where((p) => p.barcode == scannedBarcode).firstOrNull;

        if (mounted) {
          if (foundProduct != null) {
            // Product found - show options
            _showProductFoundDialog(foundProduct, scannedBarcode);
          } else {
            // Product not found - show options to add
            _showProductNotFoundDialog(scannedBarcode);
          }
        }
      } catch (e) {
        if (mounted) {
          ScaffoldMessenger.of(context).showSnackBar(
            SnackBar(
              content: Text('Error searching for product: $e'),
      ...
      builder: (context) => AlertDialog(
        ...
        content: Column(
          ...
          children: [
            ...
            Text('Product: ${product.nameEn}'),
            Text('Persian: ${product.nameFa}'),
            Text('Brand: ${product.brandEn}'),
      ...
      builder: (context) => AlertDialog(
        ...
        content: Column(
          ...
          children: [
            ...
            const Text('Would you like to add a new product?'),
          ],
        ),
        actions: [
          TextButton(
